...
S.add("CA209-7DX")
S.add("CA209-9DW")
def rename(prefix,filespath,offset):
    offset=int(offset)
    for fp in os.walk(filespath):
        for filename in fp[2]:
            if os.path.splitext(filename)[1]!=".pdf":
                continue
            file=fp[0]+os.sep+filename
            #filename=fp[2]
            protocal=ExtractKeyWords.getProtocol(filename,Tools.pdfToText(file))
            if protocal in ExtractKeyWords.ProtocalMap:
                protocal=ExtractKeyWords.ProtocalMap[protocal]
            realnumber=offset+int(filename.split('_')[0])
            newname=prefix+str(realnumber)
            if protocal not in S:
                newname+='-W'
            newname+='.pdf'
            if protocal=="1":
                newname="无法处理此文件"+filename        
            #newname=fp[0]+os.sep
            os.rename(file,fp[0]+os.sep+newname)    

def rename9DW(filespath,offset):
    rename9DW("2019236SUSAR-",filespath,offset)
#rename9DW("/mnt/c/Project/workspace/data/pdf/CA209_9DW SUSAR Case Report Feb2022",1000)

def rename9DW(FilePath):
    filePath=FilePath
    def f(nums):
        ans=[]
    ...
    for sheet in Sheets:
        ...
        for i in range(2,rows+1):
            if k1 and k2 and k0 and k3:
                ...
                if str(sheet[k3+str(i)].value).replace(" ","")in S:
                    d[k]=1            
                #第一列需要放重命名后的序号
                c1=sheet[k9+str(i)]
                if not c1:
        ...
        if len(file)>4 and file[len(file)-1:len(file)-5:-1]=='fdp.':
            ...
            if len(col)<2:
                ...
                if k not in mp:
                    ...
                    if k in d:
                        newname=filePath + os.sep+"2019236SUSAR-"+"_可能重复！_".join(mp[k])+".pdf"
                    else:
                        newname=filePath + os.sep+"2019236SUSAR-"+"_可能重复！_".join(mp[k])+"-W.pdf"
            tf.close()
